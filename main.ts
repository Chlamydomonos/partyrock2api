...
function transformRequest(req: OpenAIRequest) {
    // 标准化角色为 'user'
    for (const mes of req.messages) {
    ...
    // 合并相邻相同 role 的消息
    let mergedMessages: OpenAIMessage[] = [];
    for (let i = 0; i < req.messages.length; i++) {
        const currentMessage = req.messages[i];
        ...
        if (mergedMessages.length === 0) {
            ...
            // 检查最后一条消息的 role 是否和当前消息的 role 相同
...
app.post('/v1/chat/completions', async (c) => {
    try {
        ...
        if (!appId || !cookie || !csrfToken) {
            return c.text('Invalid Base64 or key format. Expecting "appId|||csrfToken|||cookie".', 401);
        }
